...
import { EVENT_CHAIN_V1, EVENT_CHAIN_V2 } from './EventChain.js';
...
export default class Event {
    ...
    toBinaryV1() {
        return concatBytes(this.previous, Uint8Array.from([keyTypeId(this.signKey.keyType)]), this.signKey.publicKey, convert.longToByteArray(this.timestamp), convert.stringToByteArray(this.mediaType), this.data);
    }
    toBinaryV2() {
        return concatBytes(this.previous, Uint8Array.from([keyTypeId(this.signKey.keyType)]), this.signKey.publicKey, convert.longToByteArray(this.timestamp), convert.stringToByteArrayWithSize(this.mediaType), convert.bytesToByteArrayWithSize(this.data), convert.shortToByteArray(this.attachments.length), ...this.attachments.map((a) => {
            return concatBytes(convert.stringToByteArrayWithSize(a.name), convert.stringToByteArrayWithSize(a.mediaType), convert.bytesToByteArrayWithSize(a.data));
        }));
    }
    verifySignature() {
    ...
    static from(data, version = 2) {
        ...
        try {
            ...
            if (data.signature)
                event.signature = Binary.fromBase58(data.signature);
            if (data.hash)
                event._hash = Binary.fromBase58(data.hash);
