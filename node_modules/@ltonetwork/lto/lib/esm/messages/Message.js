...
import { byteArrayToLong, byteArrayWithSizeToBytes, bytesToByteArrayWithSize, longToByteArray, stringToByteArrayWithSize, } from '../utils/convert.js';
import { DEFAULT_MESSAGE_TYPE } from '../constants.js';
export default class Message {
    constructor(data, mediaType, type = DEFAULT_MESSAGE_TYPE) {
        this.type = type;
    ...
    toBinary(withSignature = true) {
        ...
        return concatBytes(stringToByteArrayWithSize(this.type), Uint8Array.from([keyTypeId(this.sender.keyType)]), this.sender.publicKey, base58.decode(this.recipient), longToByteArray(this.timestamp.getTime()), Uint8Array.from([this._encryptedData ? 1 : 0]), data, withSignature ? this.signature : new Uint8Array(0));
    }
    toJSON() {
        const base = {
            type: this.type,
    ...
    static fromBinary(data) {
        ...
        if (signature.length > 0)
            message.signature = new Binary(signature);
        return message;
    }
}
